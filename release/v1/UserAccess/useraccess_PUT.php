...
if(database_get_row($conn, "SELECT user_id, course_id FROM user_access WHERE course_id=? AND user_id=? AND role='STUDENT'", "ss", $selectVals) == null){
    echoError($conn, 403, "AuthorizationFailed");
}

$insertVals = array($notifications, $user_id, $courseId);
database_update($conn, "UPDATE user_access SET notifications=? WHERE user_id=? AND course_id=? AND role='STUDENT' LIMIT 1", "iss", $insertVals);
