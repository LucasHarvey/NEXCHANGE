...
$notetakerNotes = getNotesForNotetaker($conn, $user_id, $sortMethod, $sortDirection, $offset); // will be empty if not a notetaker in any courses
...
function getNotesForStudent($conn, $userId, $_sortMethod, $_sortDirection, $hideDownloaded, $offset){
    ...
    $selectQuery = "SELECT  n.id, n.name, n.description, n.taken_on, n.created, nfd.lastDownloaded, ".
                    ...
                    "WHERE ua.user_id=? AND n.user_id != ua.user_id AND ua.role='STUDENT' AND ua.expires_on >= NOW()";
                    
    if($hideDownloaded){
        $selectQuery = $selectQuery . " AND nfd.lastDownloaded IS NULL";
    }
    $selectQuery = $selectQuery . getSortQuery($_sortMethod, $_sortDirection, $hideDownloaded, $offset);
    
    return database_get_all($conn, $selectQuery, "s", $userId);
}

function getSortQuery($_sortMethod, $_sortDirection, $hideDownloaded, $offset){
